...
function Google({ size = 16, titleId }: IconProps) {
	return (
		<svg
			version="1.1"
			...
			height={size}
			role="img"
			aria-labelledby={`icon-${displayName}-title`}
			data-testid={`icon-${displayName}`}>
			<title>{titleId ? <FormattedMessage id={titleId} /> : displayName}</title>
			<g id={`icon-${displayName}-Group`} stroke="none" strokeWidth="1" fill="currentColor" fillRule="evenodd">
				<path
